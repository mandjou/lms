(function() {
    ...
    var lmsAlertError = {
        template: '<div class="alerts" ng-cloak="" role="alert">' +
                        ...
                        '</div>' +
                  '</div>',
        controller: lmsAlertErrorController
    };

    angular
        .module('lmsApp')
        .component('lmsAlertError', lmsAlertError);

    lmsAlertErrorController.$inject = ['$scope', 'AlertService', '$rootScope'];

    function lmsAlertErrorController ($scope, AlertService, $rootScope) {
        var vm = this;
        ...
        var cleanHttpErrorListener = $rootScope.$on('lmsApp.httpError', function (event, httpResponse) {
            var i;
